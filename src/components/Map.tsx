...
const Map: React.FC<MapProps> = ({ 
  ...
  const renderTooltip = () => {
    ...
    return (
      <div className="absolute z-10 p-2 bg-white rounded shadow pointer-events-none"
           style={{
             left: tooltipInfo.x + 10,
             top: tooltipInfo.y + 10
           }}>
        <div><b>MMSI:</b> {tooltipInfo.object.mmsi}</div>
        <div><b>Ship Name:</b> {tooltipInfo.object.shipname}</div>
        <div><b>Speed:</b> {tooltipInfo.object.speed}</div>
        <div><b>Heading:</b> {tooltipInfo.object.heading}°</div>
      </div>
    );
  };
